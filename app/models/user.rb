...
class User < ActiveRecord::Base
  ...
  def self.find_or_create_for_oauth(auth, signed_in_resource=nil)
    ...
    user = User.create(name:     auth.extra.raw_info.try(:name),
                ...
                last_name: auth.extra.raw_info.try(:given_name),
  ...
  def refresh_token!
    if omniauth_expires_at < Time.now
      ...
      client.authorization.client_id = ENV.fetch('GOOGLE_APP_ID')
      client.authorization.client_secret = ENV.fetch('GOOGLE_SECRET_ID')
  ...
  def current_email_threads
    email_threads.includes(:emails).where("last_email_at > ?", Mail::Adapters::MailAdapter::LOOKBACK.days.ago).order("last_email_at desc")
  end
